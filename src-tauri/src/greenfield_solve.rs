...
/// CORE solve: detections + dims + budget + index dir → response. No IPC, no env
/// reads for CONFIG (the index dir arrives resolved). Constants FROZEN — the only
/// config deviations from `SolveConfig::default()` are `abort_on_accept = true` and
/// `band_major = false` (the shipped arm), plus the optional `budget_ms` override.
...
pub async fn solve_greenfield(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        solve_greenfield_core(&detections, width, height, budget_ms, &index_dir)
    })
    .await
...
mod tests {
    ...
    fn m66_solves_and_hydrates() {
        ...
        let resp = solve_greenfield_core(&dets, 2160, 3840, None, &idx_path)
