...
`.mcp.json` or `claude mcp add`). Point `args` at the **absolute** path of `server.mjs`
in your checkout:
...
```json
{
  "mcpServers": {
    "skycruncher-solve": {
      "command": "node",
      "args": ["K:/Coding Projects/Newtonian Color Engine/ASTROLOGIC_DEPLOY/tools/mcp/server.mjs"]
    }
  }
}
```
