...
export interface RunnerSession {
    ...
    /**
     * OPTIONAL teardown — release this run's heavy retained image buffers before
     * the NEXT queue item decodes. The real OrchestratorSession implements it;
     * fake test sessions may omit it (called guarded). Without this, item N's
     * ~216 MB session buffers can coexist (lazy GC) with item N+1's large-RAW
     * decode and OOM a constrained webview by item 2-3 — the same failure the
     * single-image "process another CR2" path hit. Memory hygiene only.
     */
    dispose?: () => void;
}
...
export async function runOneFile(
    ...
    // Release this run's heavy image buffers BEFORE the next queue item decodes
    // (everything the outcome needs was copied out of the session above:
    // resultFromSolution is a scalar/small copy, dispose does not touch it).
    // Guarded — fake test sessions may not implement dispose.
    try { session.dispose?.(); } catch { /* teardown must never fail the run */ }
