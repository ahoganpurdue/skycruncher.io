...
export function makeRepoDemoConnector(opts?: {
    ...
    return {
        ...
        status: 'ready',
        note: `${manifest.length} sample frame${manifest.length === 1 ? '' : 's'} included with the app.`,
        pick: async () => {
            const out: SourceFile[] = [];
 ...
 * order: the ready local / watched-folder / demo sources. `drop` is not a card
 * (it is the always-on drop zone) so it is omitted.
 *
 * HONEST-OR-ABSENT (LAW 3): the Google Drive / Dropbox connectors are NOT
 * listed here — an unbuilt feature is ABSENT from the UI, never a card that
 * narrates its own "STUB — not wired yet" status in the shipped app. The
 * `makeGoogleDriveStub` / `makeDropboxStub` factories remain exported as the
 * design SEAM (wiring the real OAuth = add the factory back here + implement
 * `pick()`, per docs/OAUTH_DRIVE_INTAKE_DESIGN.md), but nothing renders them.
 */
export function buildConnectors(): SourceConnector[] {
    return [
        ...
        makeDirectoryConnector({
            id: 'intake-dir',
            displayName: 'Watched Folder',
            description: 'Point at a folder of incoming frames; every supported frame in it is queued.',
            note: 'Watch a folder for new frames — re-pick any time.',
        }),
