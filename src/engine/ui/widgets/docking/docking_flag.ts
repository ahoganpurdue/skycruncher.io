 ...
 * for this load only (pure read, no write). RC-WALKTHROUGH BUILD: DEFAULT ON —
 * only the exact localStorage string '0' (an explicit user opt-out) turns it off.
 * Storage-unavailable ⇒ on. This default flip lives ONLY on the rc branch until
 * the owner's walkthrough ratifies it for main (look-at-the-app rule).
 */
export function getDockingEnabled(): boolean {
    ...
    try { return localStorage.getItem(DOCKING_ENABLED_STORAGE_KEY) !== '0'; }
    catch { return true; }
}
