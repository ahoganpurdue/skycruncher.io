...
import { DECODE_FAILURE_MESSAGE } from './session_replacement';
...
interface SignalGraphProps {
    ...
    /** Fired when the decode/extract FATALLY fails (null return or throw) so the
     *  App can surface an honest message instead of the historical silent wedge. */
    onError?: (message: string) => void;
}

export const SignalGraphStep: React.FC<SignalGraphProps> = ({ session, isActive, autoRun, backgroundImageUrl, onComplete, onImageClick, onError }) => {
    ...
    // Honest decode/extract failure state — replaces the historical silent wedge
    // (a null/throwing decode used to only console.error, leaving the step frozen
    // with no signal and no message). null until a failure occurs.
    ...
    const runExtraction = async () => {
        ...
        setLoading(true);
        setExtractError(null); // clear any prior failure on a fresh attempt
        ...
        try {
            ...
            if (!res) throw new Error('decode returned no sensor data');
            setSignal(res);
            setHasRun(true);
            ranThisMount.current = true;
        } catch (err) {
            // HONEST failure surface (2026-07-22): a decode/extract failure used to
            // ONLY console.error, leaving the step frozen with no message — the
            // owner's "can't run another image, nothing happens". Now the user is
            // told, and told what usually clears it (the memory-exhaustion class,
            // the measured cause of the second-large-RAW failure on a webview).
            console.error("Extraction failed:", err);
            setExtractError(DECODE_FAILURE_MESSAGE);
            try { onError?.(DECODE_FAILURE_MESSAGE); } catch { /* reporting must never re-throw */ }
        } finally {
    ...
    return (
        ...
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 min-h-0 relative bg-space-950/60 border-y border-line-subtle">
                ...
                {!hasRun && !loading && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 bg-black/40 backdrop-blur-sm">
                        {/* HONEST failure surface — no more silent wedge. The step
                            SAYS what happened and how to clear it (＋ New image in
                            the header, always available during a run). */}
                        {extractError && (
                            <div
                                data-testid="extract-error"
                                role="alert"
                                className="max-w-md text-center px-6 py-4 rounded-xl border border-danger bg-danger/10 text-danger"
                            >
                                <div className="font-bold uppercase tracking-widest text-xs mb-1">Decode failed</div>
                                <div className="text-sm opacity-90 font-sans">{extractError}</div>
                            </div>
                        )}
                        <button
                        ...
                        >
                            {extractError ? 'Retry extraction' : 'Start Signal Extraction'}
                        </button>
                    </div>
                )}
