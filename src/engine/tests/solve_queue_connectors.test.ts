...
describe('solve queue — connector seam', () => {
    it('registry lists only the ready local / watched-folder / demo sources', () => {
        const cs = buildConnectors();
        const byId = Object.fromEntries(cs.map((c) => [c.id, c]));
        // Honest-or-absent (LAW 3): the unbuilt Google Drive / Dropbox connectors
        // are NOT rendered — an unbuilt feature is ABSENT, never a card narrating
        // its own "STUB — not wired yet" status in the shipped app (owner ruling
        // 2026-07-23). The stub FACTORIES remain as the design seam (asserted below).
        expect(Object.keys(byId).sort()).toEqual(['intake-dir', 'local-dir', 'repo-demo']);
        expect(byId['gdrive']).toBeUndefined();
        expect(byId['dropbox']).toBeUndefined();
        expect(byId['repo-demo'].status).toBe('ready');
        // The demo note is plain user copy — no internal `/demo` path leak.
        expect(byId['repo-demo'].note).not.toMatch(/\/demo/);
    });

    it('the Google Drive / Dropbox stub factories remain as the (unrendered) design seam', () => {
        // The seam still exists so wiring real OAuth is add-back + implement pick();
        // it is honestly marked STUB at the factory, it is just not listed in the UI.
        expect(makeGoogleDriveStub().status).toBe('stub');
        expect(makeDropboxStub().status).toBe('stub');
        expect(makeGoogleDriveStub().note).toMatch(/stub/i);
    });
