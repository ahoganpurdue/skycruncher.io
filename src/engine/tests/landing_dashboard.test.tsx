...
describe('Landing dashboard on a null receipt (LAW 3 — honest-or-absent)', () => {
    ...
    it('WidgetDock paints the AWAITING-SOLVE voice on the landing (null receipt, default stats knob)', () => {
        // Docking is DEFAULT ON (opt-out) since @492a985a — the landing now mounts
        // the dockview surface. This test asserts the legacy-grid frame taxonomy
        // (the AWAITING-SOLVE voice in WidgetFrame), which lives on the opt-out
        // path, so explicitly disable the docking surface with the '0' flag. The
        // dock itself stays mounted (widgets.dock unset ⇒ default ON).
        installLocalStorage({ 'skycruncher.docking.enabled': '0' });
    ...
    it('WidgetDock labels scaffolds PLANNED (not-yet-built), distinct from a live absent state (R2)', () => {
        ...
        // Docking DEFAULT ON since @492a985a; this asserts the legacy-grid PLANNED
        // taxonomy, so opt out of the dockview surface with the explicit '0' flag.
        installLocalStorage({ 'skycruncher.widgets.weight': 'heavy', 'skycruncher.docking.enabled': '0' });
        try {
            const markup = renderToStaticMarkup(<WidgetDock receipt={null} />);
    ...
    it('DEFAULTS to the dockview surface (docking opt-out flag ON since @492a985a)', () => {
        // No docking-off flag ⇒ the DEFAULT-ON docking surface path is taken (this
        // is the real landing default). The surface is a React.lazy chunk, so a
        // synchronous server render resolves to its honest Suspense placeholder —
        // NOT a fabricated widget, and NOT the legacy grid.
        const markup = renderToStaticMarkup(<WidgetDock receipt={null} />);
        expect(markup).toContain('docking-surface-loading');
        expect(markup).toContain('Loading dashboard');
        // The legacy grid frame is NOT rendered on the default path.
        expect(markup).not.toContain('data-testid="widget-dock"');
    });
