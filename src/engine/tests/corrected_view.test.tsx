...
// ── RENDER-WARP SELECTION LADDER (selectRenderWarp — TPS > SIP > measured BC) ────
// The three models are ALTERNATIVE representations of ONE displacement field,
// arbitrated by the trust ladder, NEVER summed (§2 of the plan). These prove:
// admitted-TPS-beats-SIP, TPS-refused → SIP fallback, BC fallback only when neither
// SIP nor TPS exists, the no-double-correct rule (BC ignored when SIP present), and
// honest-disabled (null) when nothing qualifies.

/** Build an astrometry block loosely (cast through any — the ladder reads a subset). */
const astroSol = (astrometry: any): PlateSolution =>
    ({ astrometry } as unknown as PlateSolution);

/** An out-of-sample-admitted TPS model (control_points + matched-length weights). */
const admittedTps = (rms_after: number) => ({
    lambda: 1, scale: 100, crpix: [50, 50],
    control_points: [[0.1, 0.1], [0.2, -0.1], [-0.1, 0.2]],
    weights_x: [0.01, -0.02, 0.01], weights_y: [0.02, 0.01, -0.01],
    affine: { dx: [0, 0, 0], dy: [0, 0, 0] },
    rms_before_arcsec: 5, rms_after_arcsec: rms_after, control_count: 3,
});
const REAL_BC = { k1: -0.012, k2: 0.003 };

describe('selectRenderWarp (render-warp ladder — SELECTION not composition)', () => {
    it('no solution / no models ⇒ null (honest-disabled, warp-free STF shown)', () => {
        expect(selectRenderWarp(null)).toBeNull();
        expect(selectRenderWarp(undefined)).toBeNull();
        expect(selectRenderWarp(astroSol({ rms_arcsec: 1, distortion_detected: false }))).toBeNull();
    });

    it('rung 2: fitted SIP only ⇒ SIP (carries the RMS it arbitrates on)', () => {
        const sel = selectRenderWarp(astroSol({ rms_arcsec: 1.4, distortion_detected: true, sip: VALID_SIP }));
        expect(sel?.source).toBe('SIP');
        expect(sel?.rms_arcsec).toBe(1.4);
    });

    it('rung 1: admitted TPS whose rms BEATS the SIP fit rms ⇒ TPS', () => {
        const sel = selectRenderWarp(astroSol({
            rms_arcsec: 1.4, distortion_detected: true, sip: VALID_SIP,
            tps: admittedTps(0.6), tps_gate: { admitted: true, reason: 'ADMITTED' },
        }));
        expect(sel?.source).toBe('TPS');
        expect(sel?.rms_arcsec).toBe(0.6);
    });

    it('admitted TPS that is WORSE than SIP ⇒ SIP wins the arbitration', () => {
        const sel = selectRenderWarp(astroSol({
            rms_arcsec: 1.0, distortion_detected: true, sip: VALID_SIP,
            tps: admittedTps(2.5), tps_gate: { admitted: true, reason: 'ADMITTED' },
        }));
        expect(sel?.source).toBe('SIP');
    });

    it('REFUSED TPS (gate.admitted=false) never renders ⇒ falls back to SIP', () => {
        const sel = selectRenderWarp(astroSol({
            rms_arcsec: 1.0, distortion_detected: true, sip: VALID_SIP,
            tps: null, tps_gate: { admitted: false, reason: 'OOS_OVERFIT' },
        }));
        expect(sel?.source).toBe('SIP');
    });

    it('rung 3: no SIP + refused TPS + measured BC ⇒ BC_MEASURED fallback', () => {
        const sel = selectRenderWarp(
            astroSol({ rms_arcsec: 1.0, distortion_detected: true, tps: null, tps_gate: { admitted: false, reason: 'COVERAGE' } }),
            REAL_BC,
        );
        expect(sel?.source).toBe('BC_MEASURED');
    });

    it('NO-DOUBLE-CORRECT: SIP present ⇒ measured BC is IGNORED (never composed)', () => {
        // BC folds into SIP at the rematch; selecting BC on top would double-warp.
        const sel = selectRenderWarp(
            astroSol({ rms_arcsec: 1.0, distortion_detected: true, sip: VALID_SIP }),
            REAL_BC,
        );
        expect(sel?.source).toBe('SIP');
    });

    it('measured BC only fires with a REAL fit (NOT MEASURED / zero-k ⇒ null)', () => {
        const noAstro = astroSol({ rms_arcsec: 1, distortion_detected: false });
        expect(selectRenderWarp(noAstro, { k1: 0, k2: 0 })).toBeNull();
        expect(selectRenderWarp(noAstro, { k1: -0.01, not_measured: 'insufficient pairs' })).toBeNull();
        expect(selectRenderWarp(noAstro, REAL_BC)?.source).toBe('BC_MEASURED');
    });

    it('detectCorrectedView lights up for an admitted-TPS-only frame (generalized)', () => {
        const info = detectCorrectedView(astroSol({
            rms_arcsec: 1.0, distortion_detected: true,
            tps: admittedTps(0.5), tps_gate: { admitted: true, reason: 'ADMITTED' },
        }));
        expect(info.available).toBe(true);
        expect(info.source).toBe('TPS');
        expect(info.label).toBe(CORRECTED_VIEW_AVAILABLE);
    });
});
