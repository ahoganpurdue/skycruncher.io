 ...
 *   • docking surface is DEFAULT OFF — only the exact '1' enables it, so the
 *     flag-off path is the byte-identical legacy grid;
...
describe('docking_flag — docking enabled (DEFAULT OFF)', () => {
    it('defaults OFF when unset', () => {
        expect(getDockingEnabled()).toBe(false);
    });
    it('only the exact "1" turns it on', () => {
        localStorage.setItem(DOCKING_ENABLED_STORAGE_KEY, '0');
        expect(getDockingEnabled()).toBe(false);
        localStorage.setItem(DOCKING_ENABLED_STORAGE_KEY, 'true');   // not '1'
        expect(getDockingEnabled()).toBe(false);
...
describe('docking_flag — ?docking= per-load URL override (no write)', () => {
    ...
    it('absence of the param ⇒ existing behavior exactly (localStorage decides)', () => {
        ...
        expect(getDockingEnabled()).toBe(false);                  // unset localStorage ⇒ off
        localStorage.setItem(DOCKING_ENABLED_STORAGE_KEY, '1');
        expect(getDockingEnabled()).toBe(true);                   // persisted '1' ⇒ on
    });
});
