...
describe('solve queue — sequential runner', () => {
    ...
    it('disposes the session after a SOLVED run (frees its buffers before the next item decodes)', async () => {
        // The ship-blocker fix: without inter-item disposal, item N's ~216MB
        // session buffers coexist (lazy GC) with item N+1's large-RAW decode and
        // OOM a constrained webview by item 2-3. The runner must release them.
        let disposed = 0;
        class Disposable extends SolvingSession { dispose() { disposed++; } }
        const session = new Disposable();
        const deps: RunnerDeps = { createSession: () => session, computeSha: shaOf };
        const outcome = await runOneFile(new ArrayBuffer(8), deps);
        expect(outcome.status).toBe('solved');
    ...
    it('disposes the session even when a stage THROWS (memory released on failure too)', async () => {
        let disposed = 0;
        class DisposableThrowing extends ThrowingSession { dispose() { disposed++; } }
        const session = new DisposableThrowing();
        const deps: RunnerDeps = { createSession: () => session, computeSha: shaOf };
        const outcome = await runOneFile(new ArrayBuffer(8), deps);
        expect(outcome.status).toBe('failed');
        expect(disposed).toBe(1);
    ...
    it('a session WITHOUT dispose still runs (optional teardown is guarded)', async () => {
        // SolvingSession has no dispose — the guarded `session.dispose?.()` must
        // not break the run (back-compat with fakes and any non-disposable arm).
        const session = new SolvingSession();
        const deps: RunnerDeps = { createSession: () => session, computeSha: shaOf };
        const outcome = await runOneFile(new ArrayBuffer(8), deps);
        expect(outcome.status).toBe('solved');
    ...
    it('processQueue disposes EACH item so a multi-file batch cannot accumulate buffers', async () => {
        let disposed = 0;
        class Disposable extends SolvingSession { dispose() { disposed++; } }
        const items: QueueItem[] = [
            createQueueItem('a', 'one.fits', 4, 'drop'),
            createQueueItem('b', 'two.fits', 4, 'drop'),
            createQueueItem('c', 'three.fits', 4, 'drop'),
        ];
        const deps: RunnerDeps = { createSession: () => new Disposable(), computeSha: shaOf };
        const final = await processQueue(items, async () => new ArrayBuffer(8), deps, () => {});
        expect(final.every((it) => it.status === 'solved')).toBe(true);
        expect(disposed).toBe(3); // one dispose per processed item
    });
