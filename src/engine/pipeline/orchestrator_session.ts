...
import { detectCorrectedView, type CorrectedViewInfo } from '../ui/corrected_view';
...
export class OrchestratorSession {
    ...
    public getCorrectedViewInfo(): CorrectedViewInfo {
        return detectCorrectedView(this.solution);
    }

    /**
     * CORRECTED VIEW render (RENDER PLANE ONLY — consumes both ledgers, feeds
     * NEITHER). Re-renders the CURRENT preview float buffer through the fitted
     * SIP inverse warp so measured distortion is visually removed, returning a
     * FRESH preview data URL. Does NOT mutate previewUrl / previewFloat32 / the
     * solution / any receipt — pure render output for the toggle. Reuses the exact
     * warp + auto-stretch + URL chain the solve path uses at RENDER_APPLY_SIP.
     * Returns null on honest absence (no usable SIP / no preview / pre-solve) so
     * the caller shows NOT AVAILABLE, never a fabricated correction. It is never
     * invoked during the solve, so the pinned solves stay byte-identical.
     */
    public renderCorrectedPreviewUrl(): string | null {
        if (!this.getCorrectedViewInfo().available) return null;
        if (!(
            this.generatePreviews &&
            this.previewFloat32 &&
            this.scales &&
            this.solution?.astrometry?.sip &&
            this.solution.wcs?.crpix &&
            this.solveW > 0
        )) {
            return null;
        }
        try {
            ...
            // solve px → preview px (isotropic; guards binning via captured solveW).
            const coordScale = pW / this.solveW;
            const crpixPreviewX = this.solution.wcs.crpix[0] * coordScale;
            const crpixPreviewY = this.solution.wcs.crpix[1] * coordScale;
            const undistorted = ImageProcessor.applySipUndistort(
                this.previewFloat32, pW, pH, sip, crpixPreviewX, crpixPreviewY, coordScale
            );
            ...
            return ImageProcessor.createPreviewUrl(imageData);
        } catch {
    ...
    private async step5_CalibrateInner(): Promise<HardwareProfile> {
        ...
        // RENDER LOOP CLOSURE (PIXEL ledger, render-only, DEFAULT OFF via
        // RENDER_APPLY_SIP): re-render the wizard preview through a single SIP
        // inverse-warp so the fitted distortion is actually removed from the
        // pixels the user sees — previously SIP was fit + badged but applied to
        // nothing. Gated OFF because it rewrites previewUrl for SIP-carrying
        // frames (CR2); SeeStar has no SIP so this is a no-op regardless. Never
        // touches WCS / matched_stars / the solve numbers (all finalized above).
        const sip = this.solution.astrometry?.sip;
        // Flowchart node (recon G4): always boxed with an honest verdict —
        // APPLIED when the render actually re-warps the preview, SKIP when the
        // DEFAULT-OFF flag / SIP / preview conditions don't fire (the sacred
        // path). Behavior is byte-identical to the prior conditional block; only
        // the two additive stage events are new.
        await this.withStage('render_apply_sip', async () => {
            if (!(
                PIPELINE_CONSTANTS.RENDER_APPLY_SIP &&
                sip &&
                this.generatePreviews &&
                this.previewFloat32 &&
                this.solution!.wcs?.crpix &&
            ...
            try {
                ...
                const previewImageData = ImageProcessor.float32ToImageDataAutoStretch(undistorted, pW, pH, this.getPreviewColorTransform(), null, this.getRenderOpts());
                this.previewUrl = ImageProcessor.createPreviewUrl(previewImageData);
                console.log('[Session] RENDER_APPLY_SIP: preview re-rendered through fitted SIP undistort.');
                return { applied: true };
            } catch (err) {
                this.warn(`SIP render undistort skipped (${err instanceof Error ? err.message : String(err)}) — preview unchanged.`, 'render');
                return { applied: false };
            }
