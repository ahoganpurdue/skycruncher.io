...
export const PIPELINE_CONSTANTS = {
    ...
    // Render-layer SIP undistort (PIXEL ledger, render-only). When ON and a
    // fitted `solution.astrometry.sip` is present, the post-solve wizard
    // preview is re-rendered through a SINGLE inverse-warp that removes the
    // measured optical distortion. DEFAULT OFF: enabling it changes the
    // rendered preview pixels (previewUrl) for frames that carry SIP (CR2),
    // and the solve→preview scale mapping wants an eyeball on a real CR2
    // render before it becomes the default. Provably a no-op when no SIP is
    // present (identity return) and never touches WCS/matched_stars/solve.
    RENDER_APPLY_SIP: envIntOverride('RENDER_APPLY_SIP', 0) === 1,
