...
import { shouldConfirmNewImage } from './engine/ui/session_replacement';
...
export default function MainApp() {
  ...
  // Session-replacement lifecycle (2026-07-22 ship-blocker: the app was one-shot
  // per launch — a second large-RAW decode failed on a memory-constrained
  // webview because the outgoing session's buffers had not been released).
  //   hasExported     — did the user download this run's receipt? Gates the honest
  //                     "discard unexported results?" confirm before a new image.
  //   confirmNewImage — the honest-discard confirm dialog is open.
  //   decodeError     — an honest message when a decode/extract FAILS (previously a
  //                     silent wedge — SignalGraphStep only console.error'd). Shown
  //                     on the landing so the user knows why and can retry.
  const [hasExported, setHasExported] = useState(false);
  const [confirmNewImage, setConfirmNewImage] = useState(false);
  ...
  const handleFileSelect = useCallback((file: File, hint?: { ra: number, dec: number, label: string }) => {
    ...
    setPreviewUrl(null);
    setDecodeError(null); // fresh attempt clears any prior decode-failure notice
    setHasExported(false); // a new run has nothing exported yet
  }, []);
  ...
  // Full teardown → back to a clean landing, ready for a NEW image. The
  // disposal is the ship-blocker fix: release the outgoing session's heavy
  // image buffers (~216MB retained) BEFORE the next image decodes, so the two
  // never coexist and blow a constrained webview's memory budget (the measured
  // cause of "once I run an image once, I can't run another"; see
  // OrchestratorSession.dispose). Guarded — teardown must never throw.
  const reset = () => {
    try { sessionRef.current?.dispose(); } catch { /* teardown best-effort */ }
    ...
    setConfirmNewImage(false);
    setDecodeError(null);
  };

  // Header "New image" + the final-step "Process another image" route here.
  // Honest-or-absent: if there is a run in flight OR results the user has not
  // exported, ask before discarding (never silently destroy unsaved work).
  // Nothing to lose (fresh landing / already exported) → straight to reset.
  const requestNewImage = () => {
    if (shouldConfirmNewImage({ showWizard, hasResults: !!astroData, hasExported })) {
      setConfirmNewImage(true);
    } else {
      reset();
    }
  };

  // Export the current run's receipt (also used by the confirm dialog's
  // "Export first" affordance). Marks the run as saved so the next
  // new-image request no longer warns about losing it.
  const exportReceipt = () => {
    if (sessionRef.current) {
      savePacket(sessionRef.current.exportPacket());
      setHasExported(true);
    }
  };
    ...
    <div className="astro-app-root">
       ...
       <div className="astro-header">
          ...
          <div className="header-right">
              ...
              {/* New image — always reachable while a run is in flight or results
                  are shown (the app was one-shot per launch before this). Routes
                  through the honest-discard confirm when there is unsaved work. */}
              {(showWizard || astroData) && (
                 <button
                    className="btn-new-image"
                    data-testid="header-new-image"
       ...
       <div className="astro-content">
          ...
          {/* HONEST DECODE-FAILURE SURFACE (2026-07-22): a failed/OOM decode used
              to wedge silently (SignalGraphStep only console.error'd). Now the
              landing SAYS so and offers a retry. Shown only back on the landing
              (not over the wizard). Additive status string. */}
          {decodeError && !showWizard && (
             <div className="error-banner" role="alert" data-testid="decode-error-banner">
                ...
                <button
                   ...
                   data-testid="decode-error-dismiss"
                ...
                {(() => {
                   ...
                   return (
                      <div className="landing-dashboard" aria-label="Instrument dashboard preview">
                         ...
                         {/* Widget dock FIRST so the widgets + their ribbon are the
                             visible dashboard element at 1080p (the ribbon is
                             viewport-fixed and only paints once the surface enters
                             view — mounting it above the past-runs ReplayDashboard is
                             what brings it on-screen without scrolling). ReplayDashboard
                             (past-runs replay, honest empty state) sits below. */}
                         <React.Suspense fallback={null}>
          ...
          {showWizard && currentFile && (
            <React.Suspense fallback={
              ...
              <PipelineWizard
                  ...
                  onError={(msg) => {
                      // A fatal decode/extract failure inside the wizard. Record
                      // the honest message; the step surfaces it inline and the
                      // landing shows it after close (no more silent wedge).
                      setDecodeError(msg);
                  }}
                  onComplete={(session) => {
                      sessionRef.current = session;
                      setShowWizard(false);
                      // Reaching the dashboard means the wizard's step-7 receipt
                      // export already ran (IntegrationStep fires onComplete only
                      // from the receipt export), so this run's data IS saved — the
                      // final-step "Process another" is a smooth loop, no false
                      // "discard unexported?" nag. (The header "New image" mid-flight
                      // still confirms, because onComplete has not fired there.)
                      setHasExported(true);
          ...
          {astroData && (
             <div className="analysis-container">
                ...
                <div className="action-row">
                    {/* Primary loop point — start another image. Routes through the
                        honest-discard confirm so unexported results are never
                        silently lost (Export first is one click away, right here). */}
                    <button
                      className="btn-reset"
       ...
       {/* Honest-discard confirm — gates "New image" when there is unsaved work
           (mid-flight run or unexported results). Never silently destructive:
           Export first is offered right here. */}
       {confirmNewImage && (
          <div className="confirm-overlay" role="dialog" aria-modal="true" aria-labelledby="confirm-new-title" data-testid="confirm-new-dialog">
             <div className="confirm-card">
                <div id="confirm-new-title" className="confirm-title">Start a new image?</div>
                <p className="confirm-body">
                   Discard this session's results? Nothing is saved automatically —
                   export first if you want to keep them.
                </p>
                <div className="confirm-actions">
                   <button
                      className="btn-confirm-cancel"
                      data-testid="confirm-new-cancel"
                      onClick={() => setConfirmNewImage(false)}
                   >
                      Cancel
                   </button>
                   {astroData && sessionRef.current && (
                      <button
                         className="btn-confirm-export"
                         data-testid="confirm-new-export"
                         onClick={exportReceipt}
                      >
                         Export first
                      </button>
                   ...
                   <button
                      ...
                      data-testid="confirm-new-discard"
                      onClick={reset}
                   >
                      Discard &amp; start new
                   </button>
       ...
       <style>{`
          ...
          .astro-header {
             ...
             /* Persistent header ABOVE the pipeline wizard's full-screen modal
                (fixed inset-0 z-50) so "New image" stays clickable mid-flight —
                the whole point of an always-available bail-out. The confirm dialog
                (z-2000) still wins over both. */
             position: relative;
             z-index: 60;
          }
           .logo {
          ...
          .astro-content {
             flex: 1;
             /* min-height:0 lets this flex child be BOUNDED to the space left under
                the header instead of growing to its content height. Without it the
                landing (hero + queue + dashboard) grew past the 100vh root, which is
                overflow:hidden — so the dock/ribbon below the fold were unreachable
                (no scrollbar). Bounded here, .landing-root's max-height:100% +
                overflow-y:auto finally engage and the page scrolls. */
             min-height: 0;
             display: flex;
          ...
          .landing-root {
             ...
             flex-direction: column;
             align-items: center;
             /* Tighter vertical rhythm so the instrument dashboard (widget dock +
                ribbon) is on-screen at 1080p without scrolling — the demo laptop
                is 1920×1080 @100%. The dock's ribbon is viewport-fixed, so once
                the surface enters view the ribbon paints at the bottom edge. */
             gap: 20px;
             padding: 16px 24px 24px;
             box-sizing: border-box;
          }
          /* Every landing column shares ONE centered max-width track so the drop
             zone, the Solve Queue card grid and the dashboard sections all line up
             on the same axis (they used to center on different widths — the drop
             zone on its content width, the dashboard on 1000px — so headings read
             visibly off-centre and clipped at narrow widths). box-sizing keeps the
             padded sections inside the track instead of overflowing it. */
          .upload-container,
          .landing-dashboard {
             width: 100%;
             max-width: 1000px;
          ...
          /* Header "New image" — always reachable while working. */
          .btn-new-image {
             display: inline-flex;
             align-items: center;
             gap: 6px;
          ...
          .btn-new-image:hover {
             background: var(--sc-btn-fill);
             color: var(--sc-btn-fill-text);
             border-color: var(--sc-btn-border);
          }

          /* Honest-discard confirm dialog. */
          .confirm-overlay {
             position: fixed;
             inset: 0;
          ...
          .btn-confirm-cancel:hover { background: var(--sc-elev); }
          .btn-confirm-export {
             background: var(--sc-accent-glow);
             border: 1px solid var(--sc-accent-ring);
             color: var(--sc-accent);
          }
          .btn-confirm-export:hover { background: var(--sc-btn-fill); color: var(--sc-btn-fill-text); }
          .btn-confirm-discard {
             background: var(--sc-danger-dim);
             border: 1px solid var(--sc-danger);
             color: var(--sc-danger);
          }
          .btn-confirm-discard:hover { background: var(--sc-danger); color: var(--sc-btn-fill-text); }
       `}</style>
    </div>
  );
}
