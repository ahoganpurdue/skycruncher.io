<!-- REFERENCE · total widget-library inventory (dock + replay + tools drivers) · tags: UI/Dashboard · revision trigger: any change to src/engine/ui/widgets/registry.ts (add/remove a widget, mount-state or weightTier change, render-tech change) · created 2026-07-11 -->
...
cascade, and the tools-lane drivers that render the SAME registry headlessly. This inventory exists
because there is exactly one widget registry feeding every one of these surfaces, and it's easy to
lose track of which widgets are actually wired, which are scaffolds, and which render tech each one
uses. Code-authoritative inventory — every row cites `file:line`; anything not read directly says
**NOT VERIFIED**.
...
render }`. A rule baked into the header comment governs every entry: **data collection is decoupled
from display** — `dataSelector` is a PURE read over the already-collected receipt/event bus,
`weightTier` gates RENDER COST ONLY, never collection (`registry.ts:6-13`).
...
set**. `tools/theses/dashboard/serve.mjs` serves a separate project dashboard UI + curated docs and
does NOT render src widgets (NOT a widget driver).

## 6. Render-tech policy conformance (`D-webgpu-default`)

Policy `D-webgpu-default`: **WebGPU for widgets/dashboard going forward where it makes sense; if a
call splits WebGPU vs WebGL, prefer WebGPU; a WebGL choice ships only with a documented why.**
DOM/SVG is fine for non-GPU surfaces (policy §3 rendering rule).
...
No registry widget is orphaned — all 31 are reachable via the dock and replay slots. A larger
"craft-your-own-graph" master-graph widget is already a longer-horizon roadmap item (~70% of its
substrate already exists in the pieces cataloged here) — referenced, not re-proposed in this card.
