# TEST ARCHITECTURE (CANONICAL — owner-ratified 2026-07-10)

Successor doctrine to the two-frame regression environment. Owner framing: generalize beyond the two
pinned frames; test ground truth — "not just of images, but of pure science and large datasets."
...
ceremonies structurally tax improvement; (c) N=2 measures no RATES — the A5 sweep found 13 verify-gate
...
- Invariance laws: solve∘rotate = rotate∘solve (WCS), parity flips, centroid translation equivariance,
  quad-hash invariance under similarity transforms.
...
- Outputs MEASURED ERROR DISTRIBUTIONS, not pass/fail: astrometric residual vs truth by FOV/density/SNR;
  ...
  resolves the D19 denominator problem by construction (task #20 criterion = injection truth).
...
- Tooling: StrykerJS (vitest runner) for TS; cargo-mutants for Rust (at cutover). COST CONTROL: per-PR
  ...
  sampled full run → per-module mutation-score report. Sacred/browser e2e and the Layer-3 benchmark NEVER
  run per-mutant.
- **E2E MUTATION via synthetic frames (owner directive 2026-07-10)**: the designated MID-COST mutant
...
- Cousin lane — INPUT mutation (fuzzing): malformed/truncated FITS+CR2, hostile EXIF, corrupt headers →
  ingest boundary must fail GRACEFULLY (owner ruling pattern: graceful failure + derivation, never
  fabrication). Seeded corpus-mutation fuzzer lives in tools/, runs nightly; crashes/fabrications = bugs.

## Governance
1. PRE-REGISTRATION (D11 culture, extended): every gate threshold/bound/score bar is frozen BEFORE the
...
3. The sacred two-frame tripwire is never removed without owner ruling; it simply stops being load-bearing
   for correctness claims.
4. Sealed stratum access = release evaluation only; unsealing for development requires owner ruling +
   replacement stratum.
...
- PRE-FREEZE-SAFE (now): Layer-1 property suite MVP (pure math) · Layer-2 generator MVP (assembly of
  ...
  (feeds task #20) · mutation MEASUREMENT run (map the shadow, no gate). All via the D11 proposer →
  frozen-test pipeline.
- POST-MONDAY WAVE 1: benchmark manifest v1 (stratified ~50-100 labeled frames from A5/gauntlet seeds) +
  pre-registered statistical gates + sealed stratum sealed.
- WITH DECODER CUTOVER (#14): Layer-0 golden battery lands (already ruled) + cargo-mutants on wasm crates.
  **STATUS 2026-07-11: cutover EXECUTED (rawler default, libraw cold path) and the Layer-0 layout-contract
  battery LANDED** (`tools/contracts/check_layout_contracts.mjs` @31f0437; 90 PASS incl. the atlas_rows
  golden in the @992338d [GATES](../GATES.md) regen). cargo-mutants on the wasm crates: not yet verified landed.
...
- [NEXT_MOVES.md](../NEXT_MOVES.md) — task #20 recall-denominator criterion and the D19 finding Layer 2 is built to resolve
