<!-- REFERENCE · location/data-privacy design for the hosted results archive; revise at next receipt schema bump or when archive MVP lands · maintainer: ahogan -->
# Results Archive — Location & Data Privacy Design

**Status:** design note. This document makes no code changes: it states the tier contract and
write-path invariants only — the receipt is bit-identity-gated (§8), so schema work waits for the
next version bump. Companion to [`docs/DATA_PLATFORM.md`](../DATA_PLATFORM.md) §2, which this
document refines and partially supersedes (§9).
...
**The sky is a GPS.** A solved field gives the exact RA/Dec of the frame center. Combine that with
an exact timestamp and the reported zenith angle and the observing site falls out of spherical
astronomy:

$$\cos z = \sin\phi \sin\delta + \cos\phi \cos\delta \cos H$$

where the hour angle $H$ is fixed by time and longitude. The sky rotates ~15°/hr, so one minute of
time equals 0.25° of hour angle, or roughly 21 km of longitude at 40° latitude ($111\,\text{km}/°
\times \cos\phi$). Minute-level time rounding therefore leaves roughly 20 km of longitude
ambiguity — the scale that makes it coherent with a ~10 km GPS radius. Publish a 10 km-fuzzed GPS
point next to a millisecond timestamp and a full-precision altitude, and the fuzz is decoration:
the attacker solves for $(\phi, \lambda)$ and ignores the GPS columns entirely.

**Coherence rule:** for each tier, timestamp and airmass/alt-az precision MUST be chosen so that
the position re-derivable from *(field center, time, zenith angle)* is no tighter than the tier's
GPS radius — evaluated at the worst case (highest zenith angle) in the upload, since airmass
sensitivity $\frac{dX}{dz} = \sec z \tan z$ grows steeply toward the horizon.
...
**NEVER per-upload jitter.** Independent jitter is a statistical gift to the attacker: $N$ jittered
points average back to the true site at $\sigma/\sqrt{N}$, and astrophotographers image from home
**hundreds of times**. A 10 km jitter over 400 uploads reconstructs the backyard to ~500 m.
...
Every receipt carries its location tier / coordinate precision **as an ordinary, queryable column**. Two reasons, both essential:
...
Observer identity uses pseudonyms, not encryption or true anonymity — this section explains the
scheme and names its limits plainly.
...
Decision: the EXACT tier is **buried in advanced settings** AND requires a **per-upload confirmation**. Consent is recorded **per upload**, and the consent record stores the **version of the dialog that was shown**.
...
| Step | What | When |
|---|---|---|
| now | This doc + the write-path invariants it states. **No code changes** — the receipt is bit-identity-gated (`RECEIPT_SCHEMA_VERSION`, `src/engine/pipeline/stages/schema_versions.ts` — authoritative, cite it rather than hand-copying; the api-smoke gate requires IEEE bit-identical receipts) | immediate |
...
This table walks through each class of adversary: what data they could reach, how they would try
to attack it, and what in this design stops them.
...
| Correlation analyst / doxxer | Many uploads from one site | Average per-upload jitter back to truth ($\sigma/\sqrt{N}$ over hundreds of home uploads) | Deterministic once-per-site fuzz reused verbatim — N uploads carry the information of one (§4) |
| Correlation analyst / doxxer | Timestamps + solved WCS + airmass/alt-az | Sky-as-GPS re-derivation of $(\phi, \lambda)$ from time + zenith angle + field center | Tier contract binds time and alt-az precision to the GPS radius; coherence rule at worst-case z (§1) |
...
This section states plainly what this design does not attempt, so an absence of protection here
