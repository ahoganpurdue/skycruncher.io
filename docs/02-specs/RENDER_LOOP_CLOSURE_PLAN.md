# Render-Loop Closure Plan (design)

This plan closes the gap between what the pipeline *measures* about a lens/optics system (its
distortion field, its PSF) and what the on-screen render actually *uses*: today the render warp
and the deconvolution preview both partly ignore the frame's own measured optics. Status: design
only — not yet implemented; the seam files named below have been verified to exist as described.

**Framing law.** There is ONE displacement field $D(x,y) = \text{observed} - \text{ideal}$. SIP (`solution.astrometry.sip`), TPS (`.tps`), and measured-BC (`lens_distortion_measured`) are three *estimators* of that one field — **never summed**. There is ONE measured optic PSF (`psfField` from `psf_characterize.ts`); `rl_deconv.ts` today ignores it and uses a global stacked `buildEmpiricalKernel`, discarding its output into `PsfDeconvReport.tiles[].after` (panel only). Closure = feed measured optics into (a) the single render warp and (b) a measured-kernel deconv, each flag-gated, byte-identical when off, each guarded by `deep_verify.forcedMeasure` preservation. Warp seam is `ImageProcessor.applySipUndistort` → `orchestrator_session.ts:926-966` (call at `:957`; the doc-authorship `:844-869` pin drifted via later `withStage` instrumentation); the render buffer is `previewFloat32` (RGB), the measurement grid is `scienceBuffer` (luminance) — the two-ledger split (LAW 1).
...
- **Pixel-ledger:** measurement stays upstream (native grid, untouched); warp is render-side resample of `previewFloat32` only; TPS coords are normalized as $(p_x - \text{crpix})/\text{scale}$ — reuse the stored `scale`/`crpix`, never re-derive.
