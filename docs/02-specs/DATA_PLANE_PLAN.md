# Data-Plane Plan — Arrow / WASM / WebGPU (DRAFT for owner review, 2026-07-09)

> **STATUS: DRAFT — NOT RATIFIED.** Synthesized from 3 audits by a subagent. Every step needs owner
> sign-off before code. No Rust/gate-touching work has been done. This is a routing + sequencing draft only.
>
> **AUDIT COVERAGE:** all three legs are now audit-backed — WASM, WebGPU, **and Arrow** (the Arrow audit,
> initially errored/undelivered, was recovered 2026-07-09 and folded in below). Arrow claims are now
> file:line-cited MEASURED findings, not WASM/WebGPU intersection inferences. The `[ARROW-UNVERIFIED]` tags are retired.
...
- WASM: the solver's SIX parallel `Float64Array`s (anchorsX/Y/B, atlasRa/Dec/Mag) + the 10-col
  ...
  they are just copied at the boundary (`vector_solver.ts:70/235`, `metrology.ts:70`; `source_extractor.ts:83`). **Cold path since cutover 2026-07-21** — `vector_solver.ts`'s WASM solver kernels belong to the legacy TS/WASM solver lane retired by the greenfield Rust solver core (`test_results/greenfield_solver/NIGHT_LEDGER_2026-07-21.md`); this Arrow-spine plan now applies only to the frozen legacy engine.
...
3. **LAW 5 — Rust changes need owner sign-off + `wasm-pack build --target web --release` rebuild.**
   Every WASM plan step is a PROPOSAL. The frozen sacreds are ULP-sensitive to the exact LLVM/opt that
   built the shipped `pkg/wasm_compute_bg.wasm` — record that toolchain before any rebuild (WASM baseline §6).

4. **EVERY STEP NAMES ITS PROTECTING GATE.** The four gates, from the audits:
   - **Greenfield reference solves (headline since the 2026-07-21 full pin swap, owner-authorized: "our solver is trustworthy... rebaseline our solves per the solver that we just tested")**: `D:/AstroLogic/test_artifacts/greenfield_solver/PINNED_REFERENCE_SOLVES.json`, sentinel-gated by `greenfield_gate.mjs` (decision-core byte-exact per pin). Includes `M66_seestar` (scale=3.679184978895153"/px, matched=265, band=4, ra_deg=170.11844356557404, dec_deg=13.048758677673888 — the same field as the legacy SeeStar pin below, RA agrees to within 0.0004° across engines) among 15 SOLVED + 4 BudgetExhausted-refused production-tier pins; full bank + `m6_ab/VERDICTS.json` (11 PASS, 4 PASS-DEGRADED, 2 REFUSED-HONEST, 0 false positives) cited in `test_results/greenfield_solver/NIGHT_LEDGER_2026-07-21.md`.
...
Recovered Arrow audit (2026-07-09; every claim file:line-cited + MEASURED). Arrow is a landed SEAM, not a
live catalog format. Neither flag-gated path is reachable today: the browser seam has no `.arrow` twins
shipped (converter writes only to gitignored `test_results/atlas_arrow`, `atlas_to_arrow.mjs:54`), and the
native path has no built T1 release + an incomplete bundle (see ORCHESTRATOR NOTE). `renderlab` consumes
starplate `.arrow` for VISUALIZATION only (`tools/renderlab/arrow_loader.mjs:14`), never the solve path.

**Honest sizing (MEASURED docs — NOT the 60-900× column-native ceiling):** Arrow.gz is only **−20.5% vs
...
**Three paths (from the recovered plan_steps):**
- **Option A — RANK 1, LOW risk (recommended first move):** ship 36 `.arrow` sector twins for the CURRENT
  ...
  `byte-identical-serialization=true`) AND both e2e sacreds byte-identical. Risk LOW — per-sector JSON
  fallback survives (`adapter:210`).
- **Option C — RANK 2, = the current intended architecture:** hybrid — browser keeps JSON/Arrow sectors
  (HYG + Gaia coexist, `spec 2.1`), native desktop gains the Gaia deep tier via starplates T1. Finish the
  native leg: build the T1 release (now from the full 64-chunk Gaia harvest — the audit's `gaia_vanguard_dr3.csv`
  was deleted in housekeeping `c659cef`; the harvest supersedes it), commit the missing `t0/allsky.arrow` blob,
  run `STARPLATES_PARITY=1` + benchmark, then an owner default-ON decision (`spec 9.1 step7`). Effort
  MEDIUM-HIGH; no SeeStar risk (v1 untouched).
- **Option B — RANK 3, TRAP (NOT a drop-in):** starplates T1 REPLACING `public/atlas/sectors`. starplates is
  **pure-Gaia** (`STARPLATES_SPEC.md:716`) — it drops the HYG named stars + mag-6.8-10 gap-fill
  (`tools/atlas/README.md:24`), changing the star SET and **breaking the SeeStar `matched=272` sacred**
  (parity there is set-equality, not byte-identity, `spec:609`). Also browser-cannot-read-native. HIGH
  effort, HIGH regression risk. Do NOT treat as a swap.

> **ORCHESTRATOR NOTE — bundled-manifest sequencing (follow-up owner decision).**
> The committed manifest at `src-tauri/resources/starplates/starplates-2026.07-gdr3/manifest.json`
> (coverage **0.8636**, 10612/12288 cells, built from the truncated vanguard CSV) has **NO `t0/allsky.arrow`
> blob in resources** — so the native seed path **cannot init TODAY**. The full-harvest release now being built
> (64/64 chunks, coverage → ~1.0) publishes under the canonical id **`starplates-2026.07-gdr3`** to an EMPTY
> bucket (no immutability conflict). The committed bundled manifest will then MISMATCH the published release
> **BY DESIGN** until the bundle is refreshed — a follow-up OWNER decision. (The adjacent
> `vanguard.bin` bundle-or-retire queue item was RESOLVED **retire**, 2026-07-10: the v1 mmap reader,
> `init_catalog`/`query_catalog`, and the §9.2 parity gate were removed; JSON atlas is now the sole
> non-native catalog path — see STARPLATES_SPEC retirement notice.) **Client impact today: nil** (sync
> has no production caller and the seed is already broken).
...
- **Note:** REQUIRES owner sign-off + rebuild (touches Rust return path). Do the TS-side buffer view first if a pointer export already exists.
...
- **Risk (LOAD-BEARING):** `vite.config.ts:26-27` sets COOP=same-origin + COEP=require-corp. Cross-origin
...
- **What:** port the dominant remaining TS hot loop — catalog<->detection cross-match/verify (`solver_entry.ts:997-1062`,
  nearest-neighbor of projected catalog across many search centers, O(N·M) per center, worst on ultra-wide) + the DEFAULT-ON two-pass BC-rematch rail — into WASM as scalar-f64 with FIXED iteration/tie-break order. **Cold path since cutover 2026-07-21** — `solver_entry.ts`'s cross-match/verify loop and BC-rematch rail belong to the legacy TS/WASM solver retired by the greenfield Rust solver core (`test_results/greenfield_solver/NIGHT_LEDGER_2026-07-21.md`); this Phase-5 WASM port now targets only the frozen legacy lane.
- **Seam files:** `solver_entry.ts:997-1062`, `:1925` (`verify_astrometric_lock`), `lib.rs`, `photometry.rs`.
- **Effort:** ~code-wave + owner sign-off + rebuild.  **Risk:** HIGH.
- **Gate:** SeeStar e2e byte-identical + CR2 solved + api-smoke bit-identical receipt.
- **Why last despite highest payoff:** the **BC-rematch wrong-sign guard is LOAD-BEARING for SeeStar bit-identity**
  (CLAUDE.md). f64 TS==Rust is fine, but any reduction reorder shifts ULPs. Keep `simd128` OFF for this reduction or PROVE no autovec reassociation of the distance sums. Do NOT attempt before Phase-6 SIMD determinism is resolved.
...
- **What:** resolve the `deploy.md:16` warning `'+simd128 is not a recognized feature'`. Confirm whether the SHIPPED
  ...
  active on f64 reductions in `extract_blobs`/`fit_gaussian_2d`, that is the `-ffast-math`-class risk to the frozen sacreds.
- **Effort:** ~audit.  **Risk:** MED.  **Gate:** re-run both e2e sacreds after any rebuild.
...
- **OWNER CALL (WebGPU §7.5):** the two live cascade widgets are hand-rolled **WebGL2** (`webgl_surface.ts:189-217`),
  not WebGPU. Porting to WebGPU consolidates to one shader dialect but is owner-ratified WebGL for now — do NOT duplicate the surface in both APIs.

---

## OWNER ARBITRATION QUEUE (conflicts / open decisions)
The Arrow audit is **no longer missing** — it was recovered and folded in (ARROW LEG section), so the spine
thesis now rests on MEASURED evidence, not intersection inferences. The remaining arbitrations are:
1. **WebGL-vs-WebGPU cascade consolidation — RATIFIED WebGL-for-now (owner, 2026-07-09 "do it all").**
   Revisit only after Phase 1 proves WebView2 canvas presentation; do NOT duplicate the surface in both APIs.
2. **Bundled-manifest refresh timing — APPROVED (owner, 2026-07-09): post-publish wave.** Refresh the
   committed manifest + add the missing T0 blob once the full-harvest release is published. (The
   `vanguard.bin` RETIREMENT half of this wave LANDED 2026-07-10: the v1 reader + `init_catalog`/
...
3. **simd128 determinism verification** (Phase 6) BEFORE any WASM hot-loop port (Phase 5) — confirm the shipped
   `pkg` SIMD state is deterministic across rebuild toolchains; the BC wrong-sign guard is load-bearing for SeeStar bit-identity.
4. **COEP require-corp** on cross-origin starplate/atlas fetches (Phase 3) — the `starplates` bucket IS live
   (r2.dev public URL enabled 2026-07-09) but its CORP/CORS posture is **NOT VERIFIED** against a
