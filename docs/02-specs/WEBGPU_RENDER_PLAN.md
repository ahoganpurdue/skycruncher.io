<!-- REFERENCE · webgpu render design; revise on renderlab findings / WebView2 runtime changes -->
# WebGPU Render Plan — GPU-accelerated UI rendering (design)

This plan specifies where GPU rendering earns its keep in the UI — and, just as importantly,
where it does not — and lays out phased, flag-gated migrations for the surfaces that qualify.
Status: design note. Nothing here is scheduled beyond Phase 0, which is in flight in a sibling
worktree (`tools/renderlab`, LAW 4 incubator). This plan changes **no `src/` code**; at this
writing the dashboard lane owns `src/engine/ui` + `src/index.css` and the harvest lane owns
`tools/starplates/` — this document deliberately stays out of both. Framing laws: **LAW 4**
(prototype in `tools/`, zero app risk, thin drivers, port later behind module seams), **LAW 3**
(a missing GPU adapter renders an honest fallback message, never a broken canvas), **LAW 6**
(every new name below is brand-neutral: `renderlab`, `gpu_marks`, `render seam` — no new brand
literals).
...
Nothing ports out of renderlab until it has numbers and a phase is scheduled.
...
canonical: tsc baseline unchanged, vitest green, the pinned reference e2e runs **byte-identical**) plus
render-specific gates:

| Gate | Definition | Phases |
|---|---|---|
| **e2e byte-identity** | `run_wizard_seestar.mjs` + `run_wizard_cr2.mjs` byte-identical with the render flag ON and OFF — rendering must never change receipts (§4.5). Flag default stays OFF until this is flipped deliberately. | all |
...
3. **WebGL fallback.** This section originally ruled "two paths, not three" — **superseded by
   the standing WebGPU-default rule:** WebGPU for all widgets and dashboard items going forward
   where it makes sense; if split between the two, prefer WebGPU; if WebGL, document why. The
   live WebGL2 cascade tier (`src/engine/pipeline/ui/widgets/cascade/webgl_surface.ts:216`) is
   thereby **RATIFIED-with-documentation** — its documented why (per §7.5): a no-deps hand-rolled
   choice made for the 2.5D displacement surfaces before the two-paths rule and the WebGPU widget
   substrate were in scope, sound on its own terms at the time. The original rationale here (a
   middle tier doubles the shader surface — GLSL beside WGSL, a second context-loss lifecycle)
   stands as the argument that no NEW WebGL surface should ship without the same documented-why
   discipline. Forward rule: WebGPU-first.
...
The existing widget fleet's suite is the ratified dock architecture (authoritative): manifest
entries of `{id, title, dataSelector, weightTier(stats|chart|heavy), render}`, mounted in a
`WidgetDock` behind the flag `skycruncher.widgets.dock` (a pre-existing identifier — LAW 6
uniform-rename-later, don't churn; every *new* name in this section is neutral). The
`dataSelector` is a **pure read**; `null` means the dock renders the canonical
...
The cascade's four 2.5D surfaces are currently implemented as hand-rolled WebGL/SVG — the widget
...
- **Hand-rolled WebGL:** no deps, mature, and more universally available than WebGPU in the
  ...
  ships WebGPU, §4.1). That choice predates the two-paths rule being in scope.
...
every other row of this plan. But the existing WebGL choice for the cascade is already ratified
and this document changes no code for it: the final call on whether to migrate it rests with
whoever owns that surface. If it stays WebGL, the §7.1 cascade row becomes
"GPU-SURFACE (WebGL, existing implementation)" and this plan simply does not duplicate it.
...
- [Gates](../GATES.md) — canonical battery (tsc/vitest/pinned reference e2e) every phase must pass before porting out of renderlab
