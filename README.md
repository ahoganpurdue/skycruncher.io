...
RAW/FITS ingest, blind plate solving, lens and atmospheric correction, color calibration, PSF measurement and diagnostics, FITS/ASDF export, and a database of every measurement — each value measured, labeled approximate, or absent.
...
- Solves on your machine — your images never leave it. Star data streams down on demand, and an opt-in channel for contributing measurements back to the community database is in development.
...
[**Download SkyCruncher for Windows**](https://github.com/ahoganpurdue/skycruncher.io/releases/latest/download/SkyCruncher-setup.exe) — the installer is about 9 MB; star data downloads in-app on first run. Installed copies update automatically on launch. All versions: [releases](https://github.com/ahoganpurdue/skycruncher.io/releases).

Windows SmartScreen may warn on a new installer that isn't yet code-signed — use "More info → Run anyway". Code signing is planned.
...
The whole pipeline runs headless in Node, returning receipts bit-identical to the desktop app, and ships with a local MCP server so an AI agent can run solves and interrogate the results directly. See the [headless API reference](docs/reference/API.md) and the [MCP server reference](docs/reference/MCP.md).
